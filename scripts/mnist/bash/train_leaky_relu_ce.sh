...
save_args=()
if (( no_matrix && !opencl )); then save_args=("-s" "$MODELS_DIR/leaky_relu_ce.json"); fi

run_training \
    "LeakyReLU + CrossEntropy  |  LR=0.01  M=0.9  HL=512  epochs=30  [$backend]" \
    "$MODELS_DIR/leaky_relu_ce.log" \
    -p "$DATA_PATH" -a leaky_relu -c -r 0.01 -m 0.9 -e 30 -hl 512 \
