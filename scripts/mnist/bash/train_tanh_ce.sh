...
save_args=()
if (( no_matrix && !opencl )); then save_args=("-s" "$MODELS_DIR/tanh_ce.json"); fi

run_training \
    "Tanh + CrossEntropy  |  LR=0.05  M=0.9  HL=512  epochs=30  [$backend]" \
    "$MODELS_DIR/tanh_ce.log" \
    -p "$DATA_PATH" -a tanh -c -r 0.05 -m 0.9 -e 30 -hl 512 \
