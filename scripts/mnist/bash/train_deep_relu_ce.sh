...
save_args=()
if (( no_matrix && !opencl )); then save_args=("-s" "$MODELS_DIR/deep_relu_ce.json"); fi

run_training \
    "Deep ReLU + CrossEntropy  |  LR=0.005  M=0.9  HL=512+256+128  epochs=50  [$backend]" \
    "$MODELS_DIR/deep_relu_ce.log" \
    -p "$DATA_PATH" -a relu -c -r 0.005 -m 0.9 -e 50 -hl 512 -hl 256 -hl 128 \
