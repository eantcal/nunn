...
save_args=()
if (( no_matrix && !opencl )); then save_args=("-s" "$MODELS_DIR/sigmoid_ce.json"); fi

run_training \
    "Sigmoid + CrossEntropy  |  LR=0.05  M=0.9  HL=300  epochs=30  [$backend]" \
    "$MODELS_DIR/sigmoid_ce.log" \
    -p "$DATA_PATH" -a sigmoid -c -r 0.05 -m 0.9 -e 30 -hl 300 \
