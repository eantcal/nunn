...
public:
    ...
    struct LayerConfig {
        ...
        LayerConfig() = default;
        LayerConfig(size_t s) noexcept : size(s) {}
        LayerConfig(size_t s, Activation a) noexcept : size(s), activation(a) {}
    };
    ...
    public:
        SizeMismatchException() : std::runtime_error("Size mismatch") {}
    };
    class InvalidSStreamFormatException : public std::runtime_error {
    public:
        InvalidSStreamFormatException() : std::runtime_error("Invalid stringstream format") {}
    };
    class UserdefCostfNotDefinedException : public std::runtime_error {
    public:
        UserdefCostfNotDefinedException() : std::runtime_error("User-defined cost function not defined") {}
    };
    ...
    MlpNN(const std::vector<LayerConfig>& layers,
          double learningRate = 0.1,
          double momentum     = 0.5,
          CostFunction cf     = CostFunction::MSE);
    ...
    const std::vector<Activation>& getLayerActivations() const noexcept { return _layerActivations; }
